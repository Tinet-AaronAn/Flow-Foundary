...
**编排延迟与场景边界**（架构级约束，详见 [§4.4](#44-编排延迟与场景边界)）：

- FlowFoundry 经 **Temporal poll / Task Queue** 调度节点，**不适合**要求节点间损耗 **&lt; 1ms**、画布上大量节点需极速串行的 **realtime workflow**。
- 本地未调优环境下，纯编排开销常见 **约 40–70ms/业务节点**（poll + Workflow Task 决策）；生产调优后可压低，但仍为 **毫秒～十毫秒级**，无法达到内存状态机那种亚毫秒级切换。
...
FlowFoundry 的通用解释器（`FlowInterpreterWorkflow`）将画布上的每个 **Service Task / Script Task** 映射为一次 **Temporal Activity 往返**。Temporal 采用 **Worker 长轮询（poll）Task Queue** 的协作模型，而不是进程内函数直调。因此节点推进天然带有 **调度延迟**，与业务 Activity 本身的执行时间无关。

#### 4.4.1 延迟从哪来

每经过一个需要调度 Activity 的画布节点，至少包含：
...
因此：**仅 Temporal 编排税**（不含真实业务逻辑）在本地常见 **约 40–70ms/节点**；生产环境经 Worker 调优（更多 poller、Poller Autoscaling、同可用区部署）可压到 **约 15–25ms/节点**，但仍为 **毫秒～十毫秒级**，**无法**达到 **&lt; 1ms** 的节点间切换。

> 实测参考（本地 Docker Temporal + 宿主机 Worker）：单次 Activity 的 poll 约 20ms；稳态 Activity 执行（桩）约 15ms；首节点可能因冷启动略高。详见 [workflow-development-guide.md §2](./workflow-development-guide.md#2-适用场景与编排延迟预期)。

#### 4.4.2 不适合的场景

以下需求 **不应** 以「画布 + 通用解释器 + 每步一个 Activity」作为首选方案：

- **Realtime / 低延迟编排**：要求相邻节点间损耗 **&lt; 1ms**，或整条控制流在 **数毫秒内** 跑完大量分支。
- **高节点数、纯内存控制流**：画布上有几十～上百个 **无 I/O 的细粒度节点**，且期望总编排时间仍接近实时。
- **高频短事务流水线**：类似游戏 tick、行情风控链、硬实时设备编排等，需要进程内状态机或专用运行时。

上述场景应使用：**单 Activity 内批量处理**、**进程内状态机**、**流式计算引擎**，或 **手写 Temporal Workflow** 并在 Workflow 内合并多步逻辑（减少 Activity 次数），而不是在画布上拆成大量 Service Task。

#### 4.4.3 适合的场景

FlowFoundry + Temporal 的优势在 **耐久、可恢复、可观测、可治理**，而非亚毫秒编排：

- 外呼 / 催收 **多轮活动**（单轮业务 Activity 常为秒级～分钟级）。
- **人工任务**、审批、长 **Timer**、**Signal** 唤醒。
- 跨天 / 跨周实例、需要 **History 回放** 与审计的运行记录。
- 节点数量适中、单节点业务含义清晰，且 **业务 I/O 耗时远大于编排税** 的流程。

**选型原则**：若瓶颈在「节点太多、每步必须极快」，应 **合并节点** 或 **下沉到 Activity**；若瓶颈在「可靠、等人、等外部系统」，FlowFoundry 模型匹配。
